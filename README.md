...
- KAI 农场的真实游戏页已接入 Agent 托管：玩家可让 Agent 只走一步、连续托管或随时暂停；Agent 操作的是当前同一份农场状态，继续使用原规则、原本地存档与原交互结果，不会另开一局模拟。托管完成后，结算页会展示决策数、Skill 覆盖、视觉核对、守卫停手和本地 Memory 结果。Agent Lab 同时保留完整研究视图，支持画面/文本/RPC 状态融合、目标分解、层级规划、Skill Library、失败注入与自动恢复，并把最近 20 个已完成 Episode 保存在当前浏览器；成功轨迹会被归纳为可解释的 Skill 宏，在下一轮作为计划先验检索。可选接入独立的 Qwen2.5-VL + ScienceQA LoRA 服务读取真实像素帧；模型在线时，农场托管会在每步执行前启用视觉守卫，冲突或服务不可用都会先停手且不修改存档。规划仍使用确定性基线；本地轨迹归纳不等于模型训练，也不代表该跨域模型已具备游戏策略能力，当前未接入 LLM、MCTS 或 RL
