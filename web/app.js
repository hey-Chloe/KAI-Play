...
function nav(active) { return `<nav class="nav" aria-label="主要页面"><button type="button" class="btn ${active==='lobby'?'active':''}" data-view="lobby" ${active==='lobby'?'aria-current="page"':''}>游戏</button><button type="button" class="btn ${active==='agent'?'active':''}" data-view="agent" ${active==='agent'?'aria-current="page"':''}>Agent</button><button type="button" class="btn ${active==='history'?'active':''}" data-view="history" ${active==='history'?'aria-current="page"':''}>战绩</button><button type="button" class="btn ${active==='rules'?'active':''}" data-view="rules" ${active==='rules'?'aria-current="page"':''}>规则</button><button type="button" class="btn ${active==='friends'?'active':''}" data-action="view-friends" ${active==='friends'?'aria-current="page"':''}>好友</button></nav>`; }
...
function agentFarmPlot(game, plot, index, activePlot) {
  ...
  const stage = crop ? farmCropStage(plot) : null;
  const label = status === 'empty' ? '空地' : status === 'weed' ? '杂草' : `${crop.label} · ${status === 'ready' ? '成熟' : plot.wateredToday ? '已浇水' : `还需 ${farmRemainingDays(plot)} 日`}`;
...
function farmAgentSearchActionLabel(action) {
  ...
  if(action.type==='advance_day')return '推进到下一日';
  return '结束任务';
}
...
function agentLab() {
  ...
  const search=session.plan.search;
  const searchMaxVisits=Math.max(1,...(search?.rootChildren||[]).map((candidate)=>candidate.visits));
  const searchRows=search?.rootChildren?.slice(0,4).map((candidate)=>`<li><div><b>${esc(farmAgentSearchActionLabel(candidate.action))}</b><small>${candidate.visits} 次访问 · 价值 ${candidate.meanValue.toFixed(3)}</small></div><span aria-hidden="true"><i style="--search-share:${Math.round(candidate.visits/searchMaxVisits*100)}%"></i></span></li>`).join('')||'';
  const searchPanel=search?`<div class="agent-search-proof"><div><span>UCT TREE SEARCH</span><b>${search.rollouts} rollouts · ${search.expandedNodes} 节点</b></div><ol>${searchRows}</ol><small>固定种子 ${search.seed} · 最大深度 ${search.maxDepth}/${search.horizon} · C ${search.exploration.toFixed(3)}</small></div>`:'';
  const searchDecision=search?`<div><dt>树搜索</dt><dd>${search.rollouts} 次 rollout，选择动作访问 ${search.selectedVisits} 次，均值 ${search.selectedMeanValue.toFixed(3)}</dd></div>`:'';
  const searchMetrics=search?`<div><small>MCTS 搜索</small><strong>${session.metrics.mctsSearches}</strong></div><div><small>累计 Rollout</small><strong>${session.metrics.mctsRollouts}</strong></div><div><small>扩展节点</small><strong>${session.metrics.mctsExpandedNodes}</strong></div>`:'';
  return `<div class="shell page-shell agent-lab-page">${header('default','agent')}<main class="agent-lab-main">
    <section class="agent-lab-hero"><div><span>AGENT LAB · VLM + SEARCH</span><h1>让 Agent 自主完成<br><b>九日农场经营</b></h1><p>规则引擎既提供可复现基线，也作为可克隆搜索环境；UCT-MCTS、Skill Memory 与可选 VLM 守卫共享同一套真实状态和动作校验，像素观察会与 RPC 真值做跨域一致性判断。服务离线时明确降级，不伪造模型输出。</p></div><div class="agent-run-state is-${session.status}" role="status" aria-live="polite"><i aria-hidden="true"></i><span>${statusLabel}</span><strong>${game.day} / 9 日</strong></div></section>
    <section class="agent-capability-strip" aria-label="Agent 能力接入状态"><span class="is-ready">画面 + 文本 + RPC</span><span class="is-ready">可执行层级计划</span><span class="is-ready">失败候选排除</span><span class="is-ready">Skill Library</span><span class="is-ready">本地 Episodic Memory</span><span class="is-ready">UCT-MCTS 搜索</span><span class="${vlmReady?'is-ready':''}">${vlmStatusLabel}</span><span class="${session.visualMode==='guard'?'is-ready':''}">${visualMode.label}</span><span>RL 待实验</span></section>
    ...
    <section class="agent-workbench">
      <article class="agent-panel agent-plan"><header><span>PLAN · V${session.plan.revision}</span><h2>${search?'树搜索计划':'可执行层级计划'}</h2><small>${session.plan.candidates.length} 个合法候选 · 触发 ${esc(session.plan.trigger)}</small></header><p class="agent-subgoal"><small>当前子目标</small><b>${esc(session.plan.subgoal)}</b>${session.plan.memoryHint?`<em>${esc(session.plan.memoryHint)}</em>`:''}</p>${searchPanel}<ol>${planRows}</ol></article>
      ...
      <article class="agent-panel agent-decision"><header><span>DECISION</span><h2>当前决策摘要</h2></header><dl><div><dt>结构化观察</dt><dd>${esc(observation)}</dd></div>${searchDecision}<div><dt>Skill</dt><dd>${session.activeSkill ? esc(session.activeSkill.label) : '等待选择'}</dd></div><div><dt>动作</dt><dd>${last ? esc(last.actionLabel) : '尚未执行'}</dd></div><div><dt>反馈</dt><dd class="is-${last?.outcome || 'pending'}">${last?.outcome === 'failed' ? esc(`失败：${last.error}`) : esc(session.reflection)}</dd></div></dl><div class="agent-vlm-observer"><div><span>VLM PIXEL FRAME</span><b>${esc(vlmStatusLabel)}</b></div><canvas data-agent-vlm-frame width="512" height="340" role="img" aria-label="发送给 VLM 的当前 KAI 农场像素观察帧"></canvas><p class="${vlmResult&&!vlmResult.matched?'is-mismatch':''}">${esc(vlmCopy)}</p>${vlmResult?`<small>${esc(vlmResult.model||'Kai VLM')} · ${Math.round(vlmResult.latencyMs||0)} ms</small>`:''}</div></article>
    </section>
    <section class="agent-metric-grid" aria-label="当前评测指标"><div><small>目标进度</small><strong>${progress}%</strong></div><div><small>有效动作率</small><strong>${validRate}%</strong></div><div><small>计划版本</small><strong>V${session.plan.revision}</strong></div><div><small>备选动作</small><strong>${session.metrics.alternativeActionsConsidered}</strong></div><div><small>无效动作</small><strong>${session.metrics.invalidActions}</strong></div><div><small>恢复次数</small><strong>${session.metrics.recoveries}</strong></div><div><small>重规划</small><strong>${session.metrics.replans}</strong></div>${searchMetrics}<div><small>VLM 一致</small><strong>${vlmAgreement}</strong></div><div><small>视觉阻断</small><strong>${session.metrics.visualBlocks}</strong></div><div><small>平均延迟</small><strong>${vlmAverageLatency}</strong></div><div><small>VLM Token</small><strong>${vlmTokens||'—'}</strong></div></section>
...
function resetAgentLab(policy = state.agentLab?.policy || 'skill_memory') {
  stopAgentRun();
...
function queueAgentStep() {
  if (agentTimer) clearTimeout(agentTimer);
  agentTimer = null;
  if (!state.agentAuto || state.view !== 'agent' || agentRunFinished()) {
    state.agentAuto = false;
...
app.addEventListener('click', e => {
  ...
  if(el.dataset.view){
    ...
    stopMahjongBotSequence();
    if(state.view!=='agent') stopAgentRun();
  ...
  if(a==='agent-auto'){
    ...
    render();
    if(state.agentAuto)queueAgentStep();else stopAgentRun();
    return;
  }
  if(a==='agent-step'){
  ...
  if(a==='agent-fault'){
    state.agentInjectFailure=!state.agentInjectFailure;resetAgentLab();render();return;
  }
  if(a==='agent-policy'){
    const policy=el.dataset.agentPolicy;if(!Object.hasOwn(FARM_AGENT_POLICIES,policy))return;
    resetAgentLab(policy);render();return;
  }
  if(a==='view-friends'){
    state.view='friends';stopMahjongBotSequence();stopAgentRun();stopGameSync();stopRoomSync();render();globalThis.scrollTo?.(0,0);
