...
function agentLab() {
  ...
  return `<div class="shell page-shell agent-lab-page">${header('default','agent')}<main class="agent-lab-main">
    ...
    <section class="agent-capability-strip" aria-label="Agent 能力接入状态"><span class="is-ready">画面 + 文本 + RPC</span><span class="is-ready">可执行层级计划</span><span class="is-ready">失败候选排除</span><span class="is-ready">Skill Library</span><span class="is-ready">本地 Episodic Memory</span><span class="${vlmReady?'is-ready':''}">${vlmStatusLabel}</span><span class="${session.visualMode==='guard'?'is-ready':''}">${visualMode.label}</span><span>MCTS / RL 待实验</span></section>
