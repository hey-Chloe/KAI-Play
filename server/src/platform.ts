...
export class DouJoyPlatform {
  ...
  profile(userId: string) {
    ...
    return {
      ...
      dailyReward: this.store.dailyBeanReward(userId),
  ...
  async quickGame(userId: string) {
    ...
    if (balance < 128) throw new PlatformError(409, 'RELIEF_REQUIRED', '卡时豆不足，请先领取今日补助。');
    const user = this.store.user(userId)!;
  ...
  async startRoom(roomId: string, userId: string) {
    ...
    if (poorPlayer) throw new PlatformError(409, 'PLAYER_RELIEF_REQUIRED', `${poorPlayer.name} 卡时豆不足，需要先领取补助。`);
