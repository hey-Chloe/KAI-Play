...
test('the sixth catalog card launches KAI Sudoku immediately after 1048', () => {
  const lobby = sourceBetween(appSource, 'function lobby()', 'function nav(');
  assert.match(lobby, /6 款玩法，即刻开局/);
  assert.match(lobby, /1 款竞技 · 5 款免费训练/);
  ...
  assert.ok(lobby.indexOf('world-1048') < lobby.indexOf('world-sudoku6'));
  assert.ok(lobby.indexOf('world-sudoku6') < lobby.indexOf('world-three'));
